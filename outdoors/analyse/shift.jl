...
@chain runs begin
    ...
    @transform! :poi = first.(coalesce.(:spontaneous_end, :intervention))
    disallowmissing!(:poi)
...
# for (i, row) in enumerate(eachrow(runs))
#     @show i
#     fit_logistic(row.θ)
# end
...
plt = data(df) * mapping(:Δl => "Distance from POI (path length cm)", :absk => "k", :u, :v, col = :dance_by, row = :at_run => nonnumeric, color = :Δθ => rad2deg => "Total turn") * visual(Arrows2D) 
...
fig = pregrouped(df.lθshifted => "Distance from POI (path length cm)", df.θnormalized => rad2deg) * visual(Lines) * mapping(col = df.dance_by, row = df.at_run => nonnumeric) |> draw()#; axis = (; width = 400, height = 400, ytickformat = "{:n}°", yticks = -180:90:270, limits = ((-5, 5), nothing)))
...
m = mapping(col = df.induced => renamer(false => "Dance not induced", true => "Dance induced"))
...
m = mapping(color = df.induced => renamer(false => "Dance not induced", true => "Dance induced")) 
tracks = pregrouped(df.rotated2poi => first => "X (cm)", df.rotated2poi => last => "Y (cm)") * visual(Lines) 
angles = pregrouped(df.lθshifted => "Distance from POI (path length cm)", df.θnormalized => rad2deg => "Accumulated turn (°)") * visual(Lines)
...
draw!(fig[2,1], m * angles; axis = (; xticklabelsvisible = false, xticksvisible = false, xlabelvisible = false, xlabel = "", width = 250, height = 250, limits = ((-7, 17), (-200, 200)), yticks = -180:90:180))
draw!(fig[3,1], m3 * plt; axis = (; width = 250, height = 250, limits = ((-7, 17), nothing)))
legend!(fig[0, 1], h, orientation = :horizontal, titleposition = :left)
resize_to_layout!(fig)
