...
calib(file, start::Time, stop::Time, extrinsic::Time; checker_size::Real = 3.9, temporal_step::Real = 2, n_corners::NTuple{2, Int} = (5,8)) = calib(file, tosecond(start), tosecond(stop), tosecond(extrinsic), checker_size, temporal_step, n_corners)

function calib(file, start::Real, stop::Real, extrinsic::Real, checker_size, temporal_step, n_corners)
    ...
    files = [joinpath(path, "$t.png") for t in ts]
    push!(files, joinpath(path, "extrinsic.png"))
